...
import type { DuplicateCandidate, IdentityReview, MergeAudit, MergePreview, ProspectIdentityPreflightStatus } from '../domain/prospectIdentity.types'
...
export const prospectIdentityApi = {
  ...
  async getPreflightStatus(draftYear:number, position:string): Promise<ProspectIdentityPreflightStatus> { return (await apiService.get<ProspectIdentityPreflightStatus>(`${base}/preflight`, {draftYear, position})).data },
