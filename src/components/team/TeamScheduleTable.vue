  ...
  <div class="team-schedule-table w-full">
    <div class="list-header bg-team-primary text-team-accent p-4 rounded-lg shadow-lg mb-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <Dropdown v-model="seasonYear" :options="seasonOptions" optionLabel="label" optionValue="value" class="w-40"
          @change="reloadFirstPage" />
        <ThemedButton @click="createSchedule" label="Create Schedule" icon="pi pi-plus" variant="secondary" />
      </div>
    </div>

    <DataTable :value="scheduleStore.schedules" :loading="scheduleStore.loading" :lazy="true" paginator :rows="rows"
      :first="first" :totalRecords="scheduleStore.pagination?.total || 0" :rowsPerPageOptions="[5, 10, 20, 50]"
      @page="onPage" responsiveLayout="scroll" sortMode="multiple"
      :globalFilterFields="['gameLocation', 'gameCity', 'oppTeamConference']" class="themed-datatable">
...
<Column header="Matchup">
  <template #body="{ data }">
    <div class="matchup flex items-center gap-1">
      <!-- Away team -->
      ...
      <Column field="teamScore" header="Team Score" sortable>
        <template #body="{ data }">
          <div v-if="data.teamScore !== null && data.teamScore !== undefined" class="score-display team-score"
      ...
      <Column field="oppTeamScore" header="Opp Score" sortable>
        <template #body="{ data }">
          <div v-if="data.oppTeamScore !== null && data.oppTeamScore !== undefined" class="score-display opp-score"
...
const isWinningScore = (score1: number | undefined, score2: number | undefined) => {
  alert("score1: " + score1 + " score2: " + score2);
  if (score1 == null || score2 == null) return false
  return score1 > score2
}

defineExpose({ isWinningScore })
...
/* Directional checkmarks for winner alignment */
